...
class Vod extends Base
{
    ...
    public function data()
    {
        ...
        if(!empty($param['levels'])){
            $where['vod_levels'] = ['eq',$param['levels']];
        }        
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            if(empty($param['ck_del']) && empty($param['ck_level']) && empty($param['ck_levels']) && empty($param['ck_status']) && empty($param['ck_lock']) && empty($param['ck_hits'])
            ...
            foreach($res['list'] as  $k=>$v){
                ...
                if(!empty($param['ck_levels']) && !empty($param['val_levels'])){
                    $update['vod_levels'] = $param['val_levels'];
                    $des .= '&nbsp;推荐值：'.$param['val_levels'].'；';
                }                
    ...
    public function field()
    {
        ...
        if(!empty($ids) && in_array($col,['vod_status','vod_lock','vod_levels','vod_level','vod_hits','type_id'])){
            $where=[];
            $where['vod_id'] = ['in',$ids];
            if(empty($start)) {
