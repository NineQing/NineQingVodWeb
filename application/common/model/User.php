...
class User extends Base
{
  ...
  //      return ['code' => 1, 'msg' => '注册成功,请登录去会员中心完善个人信息'];
        ...
        if($config['user']['reg_phone_sms'] == '0'){
            ...
            $yzurl='https://haoma.baidu.com/phoneSearch?search='.$data['user_name'].'&position=&request_page=1';
            $regx='#not_found_text\">(.*)</div>#';
            $urlinfo=@file_get_contents($yzurl);
            if(preg_match($regx,$urlinfo,$re)){
                return ['code' => 1006, 'msg' => '请输入正确的手机号码！'];
            }
        ...
        $fields['user_nick_name'] = $data['user_nick_name'];
        $fields['group_id'] = $this->_def_group;
        $defgoup = $this->_def_group;
        if($defgoup==3){
          $fields['user_end_time'] = time()+3600*24*300;  
        }
        $fields['user_portrait'] = '/upload/user/0/tx.png';
        $fields['user_points'] = intval($config['user']['reg_points']);
        $fields['user_status'] = intval($config['user']['reg_status']);
        $fields['user_reg_time'] = time();
        $fields['user_reg_ip'] = $ip;
    ...
    public function upgrade($param)
    {
        ...
        if(self::isAgents($GLOBALS['user']) && $GLOBALS['user']['user_end_time'] > time()){
			return ['code'=>1005,'msg'=>'已是代理无须操作'];
		}
