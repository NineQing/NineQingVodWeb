        ...
        if($config['user']['reward_status']=='1'){
            ...
            if(!empty($user_1)){
                if($user_1['is_agents']==1){
                    model('User')->where('user_id',$user_pid)->setInc('user_gold',intval($order['info']['order_points']*$ratio));
                }else{
                    model('User')->where('user_id',$user_pid)->setInc('user_points',intval($order['info']['order_points']*$ratio));
                }
            }
            ...
            if(!empty($user_2)){
                if($user_2['is_agents']==1){
                    model('User')->where('user_id',$user_pid_2)->setInc('user_gold',intval($order['info']['order_points']*$ratio_2));
                }else{
                    model('User')->where('user_id',$user_pid_2)->setInc('user_points',intval($order['info']['order_points']*$ratio_2));
                }
            }
            ...
            if(!empty($user_3)){
                if($user_3['is_agents']==1){
                    model('User')->where('user_id',$user_pid_3)->setInc('user_gold',intval($order['info']['order_points']*$ratio_3));
                }else{
                    model('User')->where('user_id',$user_pid_3)->setInc('user_points',intval($order['info']['order_points']*$ratio_3));
                }
            }
