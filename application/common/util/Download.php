...
class Download
{
  ...
  //下载图片至服务器
    ...
    {
        preg_match_all ("/\w+\.(?:jpg|gif|jpeg|bmp|png)/U", strtolower($file_url), $arr);
        if(count($arr)>0 and count($arr[0])>0){
            $str=explode('.',$arr[0][0]);
        }else{
            return null;
        }
        $dirname = 'upload/vod/' . date("Ymd").'-1/';
        ...
        $save_to = $dirname.md5(microtime()).'.'.$file_name;
        $content = self::getCurl($file_url);
