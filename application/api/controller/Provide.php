    ...
    public function vod()
    {
        ...
        if(empty($html) || $cache_time==0) {
            ...
            if (!empty($this->_param['name'])) {
                $where['vod_name'] = ['like', $this->_param['name']];
            }
            if (!empty($this->_param['dy'])) {
                $where['vod_director'] = ['like', $this->_param['dy']];
            }
            if (!empty($this->_param['year'])) {
                $where['vod_year'] = ['like', $this->_param['year']];
            }
            if (!empty($this->_param['from'])) {
                $where['vod_play_from'] = ['like', '%'.$this->_param['from'].'%'];
            }
            if (!empty($this->_param['area'])) {
                $where['vod_area'] = ['like', '%'.$this->_param['area'].'%'];
            }
            if (!empty($this->_param['type_id'])) {
            ...
            $order = 'vod_time desc';
            $field = 'vod_id,vod_name,type_id,"" as type_name,vod_en,vod_time,vod_remarks,vod_play_from,vod_time,vod_year';
